...
#############################
#histogram of entire picture: x-axis is pixel intensity, y-axis is number of pixels in pic
#average histogram: replace each pixel with average of it and x number of neighbors
#make number of neighbors a variable
#find a peak: if value to right is greater, continue, if it's less, stop
#set threshold to 2 * peak

#peak idea #2
#compute average over given interval (5 or so)
#for each point, compute how high above the average the value is

#call resize again to make it the correct size for the neural network
...
if len(sys.argv) != 3:
    print "usage is: " + sys.argv[0] + " input.jpg output.bmp"
    sys.exit(0)
